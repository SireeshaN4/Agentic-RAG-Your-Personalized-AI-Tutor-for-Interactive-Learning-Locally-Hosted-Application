...
class RAGApplication:
    def __init__(self, retriever):
        ...
        # Define the tutor prompt template
        self.tutor_prompt = PromptTemplate(
            template="""You are a teaching assistant for question-answering tasks for a 10-year-old student in India.
        ...
        # Define the reviewer prompt template
        self.reviewer_prompt = PromptTemplate(
            ...
            "{student_answer}".
            Provide a grade from 1 (poor understanding) to 10 (perfect understanding). 
            Consider factors like clarity, accuracy, and completeness.""",
            input_variables=["student_answer"],
        )

    def run_tutor(self, lesson_title):
        plan = self.check_training_plan()
        if not plan:
    ...
    def check_training_plan(self):
        """Check if the training plan exists and is not empty."""
    ...
    def get_lesson_explanation(self, lesson_title, documents):
        ...
        try:
            ...
            print(f"Lesson Explanation: {tutor_response}")  # Print the lesson explanation
            self.context_history = f"Lesson Title: {lesson_title}\nContext: {documents}\nAnswer: {tutor_response}"
            return tutor_response
        except Exception as e:
    ...
    def generate_dynamic_questions(self, lesson_title, lesson_content):
        ...
        """
        prompt = f"""
        Based on the following lesson content, generate 3-5 questions that can be used to test the student's understanding of the material. If the question is multiple choice, provide options like a), b), c), etc. If the question is open-ended, just provide the question text.
